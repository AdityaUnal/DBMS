...
auto BufferPoolManager::DeletePage(page_id_t page_id) -> bool {
  std::scoped_lock latch(*bpm_latch_);
  auto page_id_idx = page_table_.find(page_id);
  if (page_id_idx == page_table_.end()) {
    return false;
  }
  frame_id_t frame_id = page_id_idx->second;
  if (frames_[frame_id]->page_id_ != page_id) {
    if (frames_[frame_id]->pin_count_ > 0) {
      return false;
    }
  } else if (frames_[frame_id]->pin_count_ > 0) {
    return false;
  }
...
auto BufferPoolManager::CheckedWritePage(page_id_t page_id, AccessType access_type) -> std::optional<WritePageGuard> {
  ...
  if (page_table_idx != page_table_.end()) {
    frame_id_t frame_id = page_table_idx->second;
    if (frames_[frame_id]->page_id_ == page_id) {
  ...
  return CheckedWritePageUnsafe(page_id, access_type);
}

auto BufferPoolManager::CheckedWritePageUnsafe(page_id_t page_id, AccessType access_type) -> std::optional<WritePageGuard> {
  if (free_frames_.size() > 0) {
    frame_id_t new_frame_id = free_frames_.front();
    free_frames_.pop_front();
    frames_[new_frame_id]->page_id_ = page_id;
    page_table_[page_id] = new_frame_id;
    auto guard_page = WritePageGuard(page_id, frames_[new_frame_id], replacer_, bpm_latch_, disk_scheduler_);
    {
      DiskRequest disk_request;
      disk_request.page_id_ = page_id;
      disk_request.is_write_ = false;
      disk_request.data_ = frames_[new_frame_id]->GetDataMut();
      auto promise = disk_scheduler_->CreatePromise();
      auto future = promise.get_future();
      disk_request.callback_ = std::move(promise);
  ...
  auto evictable_frames = replacer_->Size();
  if (evictable_frames > 0) {
    frame_id_t new_frame_id = replacer_->Evict().value();
    page_id_t old_page_id = frames_[new_frame_id]->page_id_;
    {
      DiskRequest disk_request;
      disk_request.page_id_ = old_page_id;
...
auto BufferPoolManager::CheckedReadPage(page_id_t page_id, AccessType access_type) -> std::optional<ReadPageGuard> {
  ...
  if (free_frames_.size() > 0) {
    frame_id_t new_frame_id = free_frames_.front();
    free_frames_.pop_front();
    auto guard_page = ReadPageGuard(page_id, frames_[new_frame_id], replacer_, bpm_latch_, disk_scheduler_);
    page_table_[page_id] = new_frame_id;
    frames_[new_frame_id]->page_id_ = page_id;
    {
      DiskRequest disk_request;
...
auto BufferPoolManager::FlushPageUnsafe(page_id_t page_id) -> bool {
  ...
  disk_request.page_id_ = page_id;
  disk_request.is_write_ = true;
  disk_request.data_ = frame->GetDataMut();
  auto promise = disk_scheduler_->CreatePromise();
  auto future = promise.get_future();
  disk_request.callback_ = std::move(promise);
...
void BufferPoolManager::FlushAllPages() {
  std::vector<page_id_t> page_ids;
  {
    ...
    for (const auto &entry : page_table_) {
      if (frames_[entry.second]->page_id_ == entry.first) {
...
auto BufferPoolManager::GetPinCount(page_id_t page_id) -> std::optional<size_t> {
  ...
  if (page_table_idx != page_table_.end()) {
    frame_id_t frame_id = page_table_idx->second;
    if (page_id == frames_[frame_id]->page_id_) {
